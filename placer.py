...
def calculate_net_hpwl(net_id: int,
    ...
    Args:
        ...
        slot_lookup: Pre-built dict mapping slot_name -> slot_dict
        pins_db: Optional pins database
        port_to_nets: Optional port-to-net mapping
    
    Returns:
        HPWL for this net in microns
    """
    positions = []
...
def calculate_incremental_delta(affected_nets: Set[int],
                                placement_old: Dict[str, Dict[str, Any]],
                                placement_new: Dict[str, Dict[str, Any]],
                                nets_dict: Dict[int, List[str]],
                                slot_lookup: Dict[str, Dict[str, Any]],
                                pins_db: Optional[Dict[str, Any]] = None,
                                port_to_nets: Optional[Dict[str, List[int]]] = None) -> float:
    """
    Calculate incremental HPWL delta for only the affected nets.
    
    This is MUCH faster than recalculating total HPWL for the entire design.
    
    Args:
        affected_nets: Set of net IDs affected by the move
        placement_old: Old placement dict
        placement_new: New placement dict
        nets_dict: Dict mapping net_id -> list of cell instances
        slot_lookup: Pre-built dict mapping slot_name -> slot_dict
        pins_db: Optional pins database
        port_to_nets: Optional port-to-net mapping
    ...
    Returns:
        Delta HPWL (new - old) in microns
    """
    delta = 0.0
    
    for net_id in affected_nets:
        old_hpwl = calculate_net_hpwl(net_id, nets_dict, placement_old, slot_lookup, pins_db, port_to_nets)
        new_hpwl = calculate_net_hpwl(net_id, nets_dict, placement_new, slot_lookup, pins_db, port_to_nets)
        delta += (new_hpwl - old_hpwl)
    
    return delta


def get_affected_nets(cells: List[str], cell_to_nets: Dict[str, Set[int]]) -> Set[int]:
    """
    Get all nets affected by moving the given cells.
    
    Args:
        cells: List of cell names that are being moved
        cell_to_nets: Pre-computed dict mapping cell_name -> set of net IDs
    
    Returns:
        Set of affected net IDs
    """
    affected = set()
    for cell in cells:
        affected.update(cell_to_nets.get(cell, set()))
    return affected
...
def find_nearest_slot_kdtree(target_x: float,
    ...
    if isinstance(indices, (int, np.integer)):
        ...
        # indices is numpy array -> convert elementwise
...
def place_design_core(fabric_db: Dict[str, List[Dict[str, Any]]],
                     netlist_graph: Dict[str, Dict[str, Any]],
                     pins_db: Dict[str, Any],
                     port_to_nets: Dict[str, List[int]],
                     net_index: Dict[int, Set[str]],
    ...
    """
    Core greedy placement algorithm using pre-loaded data.
    
    Args:
        fabric_db: Pre-loaded fabric database
        netlist_graph: Pre-loaded netlist graph
        pins_db: Pre-loaded pins database
        port_to_nets: Pre-loaded port-to-net mapping
        net_index: Pre-built net index
        cell_to_nets: Pre-computed cell-to-nets mapping
        kdtrees: Pre-built KD-trees
        slot_lists: Pre-built slot lists
...
def place_design(fabric_cells_path: str, 
    ...
    # Run core placement algorithm
    placement = place_design_core(
        fabric_db, netlist_graph, pins_db, port_to_nets,
        net_index, cell_to_nets, kdtrees, slot_lists, verbose=True
    )
    
    return placement


def place_design_with_sa_core(greedy_placement: Dict[str, Dict[str, Any]],
                              fabric_db: Dict[str, List[Dict[str, Any]]],
                              netlist_graph: Dict[str, Dict[str, Any]],
                              nets_dict: Dict[int, List[str]],
                              cell_to_nets: Dict[str, Set[int]],
                              ...
                              sa_T_final: float = 0.1) -> Dict[str, Dict[str, Any]]:
    """
    Run simulated annealing on pre-computed greedy placement using pre-loaded data.
    
    Args:
        greedy_placement: Pre-computed greedy placement
        fabric_db: Pre-loaded fabric database
        netlist_graph: Pre-loaded netlist graph
        nets_dict: Pre-extracted nets dictionary
        cell_to_nets: Pre-computed cell-to-nets mapping
        pins_db: Pre-loaded pins database
        port_to_nets: Pre-loaded port-to-net mapping
    ...
    Returns:
        Optimized placement after SA
    """
    # Run simulated annealing
    sa_placement = simulated_annealing_optimized(
        greedy_placement,
        fabric_db,
        netlist_graph,
        nets_dict,
...
def place_design_with_sa(fabric_cells_path: str, 
    ...
    # Run simulated annealing
    sa_placement = place_design_with_sa_core(
        greedy_placement,
        fabric_db,
        netlist_graph,
        nets_dict,
...
def refine_move_optimized(current_placement: Dict[str, Dict[str, Any]],
                          slot_lookup: Dict[str, Dict[str, Any]],
                          netlist_graph: Dict[str, Dict[str, Any]],
                          nets_dict: Dict[int, List[str]],
                          cell_to_nets: Dict[str, Set[int]],
                          ...
                          port_to_nets: Optional[Dict[str, List[int]]] = None) -> Tuple[Dict[str, Dict[str, Any]], float, str]:
    """
    OPTIMIZED refine move: Swap two cells of the same type.
    
    Uses incremental HPWL calculation for massive speedup.
    ...
    # OPTIMIZED: Calculate delta_cost using incremental HPWL (only affected nets)
    affected_nets = get_affected_nets([cell1, cell2], cell_to_nets)
    delta_cost = calculate_incremental_delta(
        affected_nets, current_placement, new_placement, 
        nets_dict, slot_lookup, pins_db, port_to_nets
    )
    
    return new_placement, delta_cost, 'refine'


def explore_move_optimized(current_placement: Dict[str, Dict[str, Any]],
                           fabric_db: Dict[str, List[Dict[str, Any]]],
                           slot_lookup: Dict[str, Dict[str, Any]],
                           netlist_graph: Dict[str, Dict[str, Any]],
                           nets_dict: Dict[int, List[str]],
                           ...
                           port_to_nets: Optional[Dict[str, List[int]]] = None) -> Tuple[Dict[str, Dict[str, Any]], float, str]:
    """
    OPTIMIZED explore move: Move a cell to a random slot within a window.
    ...
    Returns:
        (new_placement, delta_cost, 'explore') or fallback to refine_move_optimized
    """
    cell_names = list(current_placement.keys())
    ...
    # Get all slots of matching type within window
    available_slots = []
    used_slot_names = {cell['fabric_slot_name'] for cell in current_placement.values()}
    ...
    # If no slots in window, fallback to refine move
    if not available_slots:
        return refine_move_optimized(current_placement, slot_lookup, netlist_graph, 
                                    nets_dict, cell_to_nets, pins_db, port_to_nets)
    ...
    # OPTIMIZED: Calculate delta_cost using incremental HPWL (only affected nets)
    affected_nets = get_affected_nets([cell_to_move], cell_to_nets)
    delta_cost = calculate_incremental_delta(
        affected_nets, current_placement, new_placement,
        nets_dict, slot_lookup, pins_db, port_to_nets
    )
...
def generate_move_optimized(current_placement: Dict[str, Dict[str, Any]],
                            temperature: float,
                            window_size: float,
                            fabric_db: Dict[str, List[Dict[str, Any]]],
                            slot_lookup: Dict[str, Dict[str, Any]],
                            ...
                            P_explore: float = 0.3) -> Tuple[Dict[str, Dict[str, Any]], float, str]:
    """
    OPTIMIZED move generation with incremental HPWL.
    ...
    if move_type == 'refine':
        ...
        return explore_move_optimized(current_placement, fabric_db, slot_lookup, 
                                     netlist_graph, nets_dict, cell_to_nets,
                                     window_size, pins_db, port_to_nets)


def simulated_annealing_optimized(initial_placement: Dict[str, Dict[str, Any]],
                                  fabric_db: Dict[str, List[Dict[str, Any]]],
                                  netlist_graph: Dict[str, Dict[str, Any]],
                                  nets_dict: Dict[int, List[str]],
                                  cell_to_nets: Dict[str, Set[int]],
                                  ...
                                  P_explore: float = 0.3) -> Dict[str, Dict[str, Any]]:
    """
    OPTIMIZED Simulated Annealing with incremental HPWL calculation.
    
    KEY OPTIMIZATIONS:
    1. Pre-build slot_lookup once (not on every move)
    2. Use incremental HPWL (only calculate affected nets)
    3. Pass cell_to_nets for fast net lookup
    
    Expected speedup: 10-100x depending on design size
    ...
    Args:
        ...
        alpha: Cooling rate (default: 0.95)
        T_final: Final temperature (default: 0.1)
        moves_per_temp: Number of moves per temperature step (default: 100)
    ...
    Returns:
        Best placement found
    """
    # PRE-BUILD SLOT LOOKUP (CRITICAL OPTIMIZATION)
    print("Pre-building slot lookup table...")
    slot_lookup = {}
    for slots in fabric_db.values():
        for slot in slots:
            slot_lookup[slot['name']] = slot
    print(f"Slot lookup table built: {len(slot_lookup)} slots indexed")
    ...
    print(f"\n{'='*60}")
    print("OPTIMIZED Simulated Annealing with Incremental HPWL")
    print(f"{'='*60}")
    print(f"Initial HPWL: {current_hpwl:.2f} um")
    print(f"Initial Temperature: {T:.2f}")
    print(f"Cooling Rate (alpha): {alpha}")
    ...
    print(f"Window Cooling Rate (beta): {beta}")
    print(f"Move Probabilities: Refine={P_refine:.1%}, Explore={P_explore:.1%}")
    print(f"Optimization: Incremental HPWL enabled (affects only changed nets)")
    print(f"{'='*60}\n")
    ...
    while T > T_final:
        ...
        # Attempt N moves at this temperature
        for move_idx in range(moves_per_temp):
            # Generate move using optimized functions
            new_placement, delta_cost, move_type = generate_move_optimized(
                current_placement, T, W, fabric_db, slot_lookup, netlist_graph, 
                nets_dict, cell_to_nets, pins_db, port_to_nets, P_refine, P_explore
            )
            ...
            if should_accept_move(delta_cost, T):
                ...
                if current_hpwl < best_hpwl:
                    ...
                    best_hpwl = current_hpwl
                    if iteration % 10 == 0:
                        print(f"  → New best HPWL: {best_hpwl:.2f} um (Δ={delta_cost:.2f} um)")
        
        # Calculate acceptance rate
        acceptance_rate = acceptance_count / moves_per_temp if moves_per_temp > 0 else 0.0
        
        # Print progress
        if iteration % 10 == 0 or T <= T_final * 2:
            refine_rate = (refine_acceptances / refine_attempts * 100) if refine_attempts > 0 else 0.0
            explore_rate = (explore_acceptances / explore_attempts * 100) if explore_attempts > 0 else 0.0
            improvement_pct = (improvement_count / moves_per_temp * 100) if moves_per_temp > 0 else 0.0
            
            print(f"T={T:.2f} | W={W:.1f}um | Current={current_hpwl:.2f} | "
                  f"Best={best_hpwl:.2f} | Accept={acceptance_rate:.1%} | "
                  f"Improve={improvement_pct:.1f}% | "
                  f"R:{refine_rate:.0f}% E:{explore_rate:.0f}%")
    ...
    improvement_pct = (improvement / initial_hpwl) * 100
    print(f"Improvement: {improvement:.2f} um ({improvement_pct:.2f}%)")
    print(f"Total Iterations: {iteration}")
    print(f"{'='*60}\n")
...
def generate_output_dir_name(design_name: str, enable_sa: bool, 
                             sa_alpha: float = None, 
                             sa_moves: int = None, 
                             sa_T_final: float = None) -> str:
    """
    Generate output directory name based on parameters.
    
    Args:
        design_name: Name of the design
        enable_sa: Whether SA is enabled
        sa_alpha: SA cooling rate (if SA enabled)
        sa_moves: SA moves per temperature (if SA enabled)
        sa_T_final: SA final temperature (if SA enabled)
    
    Returns:
        Directory name string
    """
    if not enable_sa:
        return f"{design_name}/greedy"
    else:
        # Format: sa_alpha0.8_moves100_Tfinal0.1
        return f"{design_name}/sa_alpha{sa_alpha}_moves{sa_moves}_Tfinal{sa_T_final}"


def batch_place_design(fabric_cells_path: str,
                       design_path: str,
                       pins_path: str = 'fabric/pins.yaml',
                       base_output_dir: str = 'build') -> None:
    """
    Run batch placement with multiple SA parameter combinations.
    ...
    # Format: (enable_sa, alpha, moves, T_final)
    param_combinations = [
        # Without SA (greedy only)
        (False, None, None, None),
        # With SA - various parameter combinations
        (True, 0.8, 50, 0.1),
        (True, 0.8, 100, 0.1),
        (True, 0.8, 200, 0.1),
        (True, 0.85, 50, 0.1),
    ...
    print(f"\n{'='*60}")
    print(f"Batch Placement: {design_name}")
    print(f"Total runs: {total_runs}")
    print(f"{'='*60}\n")
    
    # Validate design once
    print("Validating design against fabric...")
    if not validate_design(fabric_cells_path, design_path):
        print("\nERROR: Design validation failed. Cannot proceed with placement.")
        return
    ...
    # Pre-load all data once (shared across all runs)
    print("Loading design and fabric data...")
    fabric_db = parse_fabric_cells(fabric_cells_path)
    logical_db, netlist_graph = parse_design(design_path)
    pins_db = parse_pins(pins_path)
    port_to_nets = get_port_to_net_mapping(design_path)
    ...
    # Build optimization data structures once
    print("Building optimization indices...")
    net_index = build_net_index(netlist_graph)
    cell_to_nets = precompute_cell_nets(netlist_graph)
    kdtrees, slot_lists = build_slot_spatial_index(fabric_db)
    ...
    # Run greedy placement once (shared starting point for all SA runs)
    print("Running greedy placement (will be reused for all SA runs)...")
    greedy_start_time = time.time()
    greedy_placement = place_design_core(
        fabric_db, netlist_graph, pins_db, port_to_nets,
        net_index, cell_to_nets, kdtrees, slot_lists, verbose=True
    )
    ...
    if greedy_placement is None:
        print("ERROR: Greedy placement failed. Cannot proceed.")
        return
    
    print(f"\nGreedy placement completed in {greedy_time:.2f}s\n")
    
    successful = 0
    failed = 0
    results = []  # Store results for summary
    
    for idx, (enable_sa, alpha, moves, T_final) in enumerate(param_combinations, 1):
        # Generate output directory name
        if enable_sa:
            run_name = f"sa_alpha{alpha}_moves{moves}_Tfinal{T_final}"
            print(f"[{idx}/{total_runs}] Running: SA (alpha={alpha}, moves={moves}, T_final={T_final})")
        else:
            run_name = "greedy"
            print(f"[{idx}/{total_runs}] Running: Greedy only (no SA)")
        
        output_dir = os.path.join(base_output_dir, design_name, run_name)
        os.makedirs(output_dir, exist_ok=True)
        
        map_output_path = os.path.join(output_dir, f"{design_name}.map")
        json_output_path = os.path.join(output_dir, f"{design_name}_placement.json")
        
        try:
            # Run placement
            if enable_sa:
                # Start timing for SA only
                start_time = time.time()
                
                # Use pre-computed greedy placement and run SA only
                placement = place_design_with_sa_core(
                    greedy_placement,
                    fabric_db,
                    netlist_graph,
                    nets_dict,
                ...
                # For greedy-only run, use the pre-computed greedy placement
                placement = {k: v.copy() for k, v in greedy_placement.items()}
                # Use the greedy time we already measured
                elapsed_time = greedy_time
            
            if placement is None:
                print(f"  ERROR: Placement failed for {run_name}")
                failed += 1
                continue
            
            # Calculate total HPWL
            total_hpwl = calculate_total_hpwl(
            ...
            if not success:
                print(f"  ERROR: Failed to write placement map for {run_name}")
                failed += 1
                continue
            ...
            if elapsed_time < 60:
                ...
                time_str = f"{elapsed_time/60:.2f}m ({elapsed_time:.2f}s)"
            else:
                hours = int(elapsed_time // 3600)
                minutes = int((elapsed_time % 3600) // 60)
                seconds = elapsed_time % 60
                time_str = f"{hours}h {minutes}m {seconds:.2f}s ({elapsed_time:.2f}s)"
            
            print(f"  ✓ Success: {output_dir}")
            if enable_sa:
                print(f"    Runtime: {time_str} (SA only, greedy was {greedy_time:.2f}s)")
            else:
                print(f"    Runtime: {time_str} (greedy only)")
            ...
            print(f"  ERROR: Exception during placement for {run_name}: {e}")
            failed += 1
            import traceback
            traceback.print_exc()
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"Batch Placement Summary")
    print(f"{'='*60}")
    print(f"Total runs: {total_runs}")
    print(f"Successful: {successful}")
    print(f"Failed: {failed}")
    print(f"Output base directory: {base_output_dir}/{design_name}/")
    print(f"{'='*60}\n")
    
    # Detailed results table
    if results:
        print(f"{'Run Name':<40} {'Runtime':<20} {'HPWL (um)':<15}")
        print(f"{'-'*75}")
        for result in results:
            ...
            print(f"{run_name:<40} {time_str:<20} {hpwl:<15.2f}")
        
        # Find best HPWL
        if results:
            best_result = min(results, key=lambda x: x['hpwl'])
            print(f"\n{'='*60}")
            print(f"Best HPWL: {best_result['hpwl']:.2f} um")
...
def main():
    ...
    parser.add_argument('--batch', action='store_true',
                        help='Run batch placement with multiple SA parameter combinations')
    parser.add_argument('--batch-output', default='build',
                        help='Base output directory for batch mode (default: build)')
