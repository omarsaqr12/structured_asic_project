#!/usr/bin/env python3
"""Run X-Tree CTS for 6502 design - both greedy and best SA placements"""
import os
import sys
import shutil
from cts_xtree import (synthesize_clock_tree, save_cts_tree, save_buffer_mapping,
...
placements = {
    'greedy': {
        ...
        'output_dir': 'build/6502/cts/greedy_xtree'
    },
    'best': {
        ...
        'output_dir': 'build/6502/cts/best_xtree'
    }
}
...
def run_cts_for_placement(name: str, config: dict):
    ...
    print(f"\n{'=' * 60}")
    print(f"Running X-Tree CTS for {name.upper()} placement")
    print(f"{'=' * 60}")
...
print("=" * 60)
print("Running X-Tree CTS for 6502 design - Both Placements")
print("=" * 60)
...
# Summary
print("\n" + "=" * 60)
print("X-Tree CTS Synthesis Summary")
print("=" * 60)
...
if all(results.values()):
    print("\n✓ All X-Tree CTS runs completed successfully!")
    sys.exit(0)
else:
    print("\n✗ Some X-Tree CTS runs failed!")
    sys.exit(1)
